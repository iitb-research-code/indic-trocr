...
model = VisionEncoderDecoderModel.from_pretrained("/home/pageocr/trocr/kannada/checkpoints/checkpoint-6000")
...
def preview(image_path):
    ...
    # plt.imshow(image)
    file = open("/home/pageocr/trocr/kannada/results/result.txt", "a")
    text = generated_text + ',' + img
    file.write(text)
    print(generated_text, ",", img)

with open("/data/BADRI/IHTR/testset_small/kannada/test.txt") as f:
    for line in f:
        img = line[5:]
        image_path = "/data/BADRI/IHTR/testset_small/kannada/images/{}".format(img.strip('\n'))
