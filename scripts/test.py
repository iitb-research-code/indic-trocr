...
def preview(image_path):
    ...
    # plt.imshow(image)
    file = open("/home/pageocr/trocr/gujarati/results/result.txt", "a")
    text = generated_text + ',' + img
    file.write(text)
    print(generated_text, ",", img)

with open("/data/BADRI/IHTR/testset_small/gujarati/text.txt") as f:
    for line in f:
        img = line[5:]
        image_path = "/data/BADRI/IHTR/testset_small/gujarati/images/{}".format(img.strip('\n'))
